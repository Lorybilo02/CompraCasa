...
public class Apartment extends Property {
    ...
    public Apartment(int id, User owner, String name, String description, String location, float latitude, float longitude,
                     String condition, String energeticClass, String heatingSystem, String imageLink, int floor, int sqm, int price,
                     int numRooms, int numBathrooms, int numKitchens, boolean hasLivingRoom, int numElevators,
                     int numInternalParkingSpace, boolean isAccessibleToDisabled, boolean isFurnished) {
        super(id, owner, name, description, location, latitude, longitude, condition, energeticClass, heatingSystem, floor, "A", imageLink, sqm, price);
        this.numBedrooms = numRooms;
        this.numBathrooms = numBathrooms;
        this.numKitchens = numKitchens;
        this.hasLivingRoom = hasLivingRoom;
        this.hasElevator = numElevators >= 1;
        this.hasParkingSpace = numInternalParkingSpace >= 1;
    ...
    public PreparedStatement getInsertQuery(Connection con) throws SQLException {
        String query = "INSERT INTO Property(owner_username, name, description, location, latitude, longitude, condition, energeticClass, heatingSystem, floor, type, image, sqm, price, " +
            "numRooms, numBathrooms, numKitchens, hasLivingRoom, numElevators, numInternalParkingSpaces, isAccessibleToDisabled, isFurnished) " +
            "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id";
